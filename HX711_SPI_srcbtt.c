/* SOURCEBITT: 2020-09-17. Edited code a little to have it compile with no errors on Raspberry Pi 4B reading a scale. */

#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
#include <unistd.h> //SOURCEBITT: added header to enable usleep()
...
int main(void) {
	spi_t *spi; //SOURCEBITT: Added * pointer as per working cperiphery expample
	...
	long tmp_avg=0;
	long tmp_avg2;
    int get_bits(); //SOURCEBITT: added statement to resolve errors

    spi = spi_new(); //SOURCEBITT: added statement as per working cperiphery example
...
while(1)
{
	...
	printf("Value: %d grams\n", (desired_bits - offset)/SCALE); //SOURCEBITT: changed scale to SCALE to match the definition
